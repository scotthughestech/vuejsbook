...
Vue.component('purchases-table', {
  ...
  <p v-else>There are currently no purchases.</p>
  `,
  methods: {
...
var app = new Vue({
  ...
  created: function () {
    self = this;
    $.ajax({
      url: 'https://api.jsonbin.io/b/YOURID/latest',
      type: 'GET',
      headers: { 'secret-key': 'YOURSECRETKEY' },
      success: function (data) {
        if (data.length == 1) {
          purchase = data[0];
          if (purchase.date == '1843-01-01' && purchase.price == '0' && purchase.description == 'Placeholder') {
            return;
          }
        }
        self.purchases = data;
  ...
  methods: {
    ...
    updateServer: function () {
      ...
      $.ajax({
        url: 'https://api.jsonbin.io/b/YOURID',
        type: 'PUT',
        headers: { 'secret-key': 'YOURSECRETKEY' },
